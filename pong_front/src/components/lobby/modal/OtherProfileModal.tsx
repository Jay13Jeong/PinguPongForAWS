...
function OtherProfileModal() {
    ...
    useEffect(() => {
        ...
        const getRank = async () =>{
            try{
                const res = await axios.get('/api/user/rank/' + (showModal.userId !== 0 ? showModal.userId : userInfo.id) , {withCredentials: true}) //쿠키와 함께 보내기 true.
    ...
    const getAvatarData = async (id : any) => {
        try{
            const avatarDataRes = await axios.get('/api/user/avatar/' + id, {withCredentials: true, responseType: 'blob'}) //blob : 파일전송용 큰 객체타입.
    ...
    async function initRelate() {
        try{
            const res = await axios.get('/api/friend/relate/' + userInfo.id, {withCredentials: true});
            setRelate(res.data);
        }catch{
            //nothing relate...
        }
    ...
    async function initUserInfo(userId: number){
        try{
            const res = await axios.get('/api/user/' + showModal.userId, {withCredentials: true});
    ...
    async function handleFollow(event: React.MouseEvent<HTMLElement>) {
        ...
        try{
            const res = await axios.post('/api/friend', {otherID : userInfo.id}, {withCredentials: true})
    ...
    async function handleUnfollow(event: React.MouseEvent<HTMLElement>) {
        event.preventDefault();
        try{
            const res = await axios.patch('/api/friend', {otherID : userInfo.id}, {withCredentials: true})
    ...
    async function handleBlock(event: React.MouseEvent<HTMLElement>) {
        event.preventDefault();
        try{
            await axios.post('/api/friend/block', {otherID : userInfo.id}, {withCredentials: true})
    ...
    async function handleUnblock(event: React.MouseEvent<HTMLElement>) {
        event.preventDefault();
        try{
            await axios.patch('/api/friend/block', {otherID : userInfo.id}, {withCredentials: true})
