...
function ProfileEditModal(props: {name: string}) {
    ...
    useEffect(() => {
        // setUserInfo();
        axios.get('/api/fa2/status', {withCredentials: true}) //쿠키와 함께 보내기 true.
        .then(res => {
            if (res.data){
    ...
    function handleSubmit(event : any) {
        ...
        axios.patch('/api/user', {username : username}, {withCredentials: true})
        .then(res => {
            //변경 성공.
            resetState();
        })
    ...
    function handle2FASubmit(event : any) {
        event.preventDefault();
        axios.patch('/api/fa2', {}, {withCredentials: true})
        .then(res => {
          if (res.status === 200){
    ...
    function handleOff2FASubmit(event : any) {
        event.preventDefault();
        axios.delete('/api/fa2', {withCredentials: true})
        .then(res => {
          if (res.status === 200){
    ...
    function handleFileSubmit(event : any) {
		...
		{
			//update Avatar here
			axios.post('/api/user/avatar', {file: inputRef.current.files![0]}, {withCredentials: true, headers: { 'Content-Type': 'multipart/form-data' }})
			.then((res) => {
