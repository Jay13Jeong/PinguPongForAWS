...
function FriendModal() {
    ...
    const [data] = useGetData('/api/user');
    ...
    useEffect(() => {
        ...
        const callFriendData = async () => {
            try{
                const res = await axios.get('/api/friend', {withCredentials: true}) //쿠키와 함께 보내기 true.
    ...
    const handleAddFriendSubmit = async (event : any) => {
        ...
        try{
            const res = await axios.post('/api/friend/name', {username : target}, {withCredentials: true});
