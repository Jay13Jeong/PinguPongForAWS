  ...
  const navigate = useNavigate();
    ...
    useEffect(() => {
      ...
      const get2faStatus = async () => {
        try{
          const res = await axios.get('/api/fa2/status', {withCredentials: true}) //쿠키와 함께 보내기 true.
          if (res.data.twofa === false){
            navigate(RoutePath.lobby); //2단계인증이 꺼져있으면 로비로 간다.
            return ;
          }
          try{
            await axios.get('/api/fa2',{withCredentials: true});
          } catch {
    ...
    function handleSubmit(event : any) {
      ...
      const submitCode = async () => {
        try{
          const res = await axios.post('/api/fa2', {code : code}, {withCredentials: true})
          if (res.status === 200)
            navigate('/lobby');
        }catch(err: any){
          setError(true);
